...
#adding geo-points
napoleonCityCoords <- napoleonCoords + geom_point(data = cities, aes(x = long, y = lat),
		 color = cityLabelColor) 
		 
#city labels  		 
napoleonCityText <- napoleonCityCoords + geom_text_repel(data = cities, aes(x = long, y = lat, label = city),
